...
export class TSDParser {
    ...
    parseElementFromString(): TSDElement {
        this.remainingStr = this.remainingStr.trim();
        let keyEndIndex = this.remainingStr.indexOf(":");
        if (keyEndIndex == -1) throw new Error(`":" expected. ${this.remainingStr}`);
        ...
        let value: TSDElement | TSDElement[] | string | number | null = null;
        switch (true) {
        ...
        if (newElement.value != null && newElement.value.constructor == [].constructor) {
            (newElement.value as TSDElement[]).forEach(element => {
...
export class ParserHelper {
    static findClosingBracketIndex(str: string, openingIndex: number) {
        let i = 1;
